...
from .runtime import create_run_dir, safe_table, validate_image_batch
...
def run_once_cli(argv: list[str]) -> int:
    ...
    try:
        ...
        if image_index + 1 >= len(argv):
            log("Usage: colonynet --run-once IMAGE_PATH [--output OUTPUT_DIR]")
            return 2
