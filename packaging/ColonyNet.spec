...
ROOT = SPEC_DIR if (SPEC_DIR / "src").exists() else SPEC_DIR.parent
APP = ROOT / "packaging" / "entrypoint.py"
MODELS = ROOT / "src" / "colonynet" / "models"
...
for model_path in MODELS.glob("*.pt"):
    datas.append((str(model_path), "colonynet/models"))
